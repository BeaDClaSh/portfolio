...
const AnimatedBackground: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            // Обрабатываем каждое "blob" с использованием рефов
            blobRefs.current.forEach((blob, index) => {
                if (blob) {
                    const initialPos = initialPositions[index];

                    // Расчет смещения по осям X и Y
                    const xOffset = Math.sin(newScroll / 100 + index * 0.5) * 340; // Горизонтальное движение
                    const yOffset = Math.cos(newScroll / 100 + index * 0.5) * 40;  // Вертикальное движение
